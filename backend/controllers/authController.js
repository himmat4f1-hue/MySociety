...
const makeUniqueSlug = async (base) => {
  ...
  while (await Society.findOne({ slug })) {
    slug = `${slugify(base)}-${n}`;
    n++;
  }
  return slug;
...
const issueSessionResponse = async (user, membership) => {
  const society = await Society.findById(membership.society);
  const token = generateToken({ id: user._id, societyId: society._id, role: membership.role, flatId: membership.flatId || null });
  return {
    _id: user._id,
    name: user.name,
    email: user.email,
    ...
    avatar: user.avatar,
    society: { _id: society._id, name: society.name, slug: society.slug, isGuestSandbox: society.isGuestSandbox, expiresAt: society.expiresAt },
    token,
  };
};
...
const registerSociety = asyncHandler(async (req, res) => {
  ...
  const plan = planSlug ? await Plan.findOne({ slug: planSlug }) : null;
  ...
  if (type === 'Apartment') {
    await provisionUnits(society._id, Number(buildingsCount), Number(flatsPerBuilding));
  } else {
    await provisionUnits.provisionHouses(society._id, Number(housesCount));
  }
  ...
  const existingMembership = await Membership.findOne({ user: user._id, society: society._id, role: 'chairman' });
  if (existingMembership) {
    return res.status(400).json({ message: 'You are already the Chairman of this society' });
  }

  const membership = await Membership.create({ user: user._id, society: society._id, role: 'chairman' });
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  let memberships = await Membership.find({ user: user._id, status: 'active' }).populate('society');
  ...
  if (societyId) {
    memberships = memberships.filter((m) => m.society._id.toString() === societyId);
  }
  const distinctSocieties = [...new Map(memberships.map((m) => [m.society._id.toString(), m.society])).values()];
  if (distinctSocieties.length === 0) {
    return res.status(404).json({ message: 'No matching society membership found.' });
  }
  if (distinctSocieties.length > 1) {
    return res.json({
      step: 'society',
      options: distinctSocieties.map((s) => ({ societyId: s._id, name: s.name, slug: s.slug })),
    });
  }
  const chosenSociety = distinctSocieties[0];
...
const guestLogin = asyncHandler(async (req, res) => {
  ...
  await seedGuestSandbox(society._id, user._id);
...
const getMe = asyncHandler(async (req, res) => {
  res.json({
    ...
    society: { _id: req.society._id, name: req.society.name, slug: req.society.slug, isGuestSandbox: req.society.isGuestSandbox, expiresAt: req.society.expiresAt },
  });
});
...
const getMySocieties = asyncHandler(async (req, res) => {
  const memberships = await Membership.find({ user: req.user._id, status: 'active' }).populate('society');
  res.json(
    memberships.map((m) => ({
      societyId: m.society._id,
      name: m.society.name,
      slug: m.society.slug,
      role: m.role,
      flatId: m.flatId,
