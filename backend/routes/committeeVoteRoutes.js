...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existing = await CommitteeVote.findOne({
      where: { society: req.societyId, electionDate, voterFlatId: req.flatId },
    });
    if (existing) {
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const voted = await CommitteeVote.findOne({ where: { society: req.societyId, electionDate, voterFlatId: req.flatId } });
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const votes = await CommitteeVote.findAll({ where });
    const tally = {};
    votes.forEach((v) => {
