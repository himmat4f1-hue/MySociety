...
const seed = async () => {
  ...
  await Promise.all([
    User.deleteMany(), Society.deleteMany(), Plan.deleteMany(), Membership.deleteMany(),
    Unit.deleteMany(), Building.deleteMany(), Resident.deleteMany(), Pet.deleteMany(), Visitor.deleteMany(),
    Complaint.deleteMany(), Maintenance.deleteMany(), Notice.deleteMany(), Amenity.deleteMany(),
    Document.deleteMany(), Invoice.deleteMany(), Transaction.deleteMany(), Meeting.deleteMany(),
    Poll.deleteMany(), Emergency.deleteMany(), CameraRequest.deleteMany(), Policy.deleteMany(),
    Investment.deleteMany(), Fund.deleteMany(), GatePass.deleteMany(), Shift.deleteMany(),
    Task.deleteMany(), Supply.deleteMany(), Lease.deleteMany(),
    FlatOwner.deleteMany(), FamilyMember.deleteMany(), Vehicle.deleteMany(), HomeService.deleteMany(),
    RoleChecklist.deleteMany(), AgendaItem.deleteMany(), MeetingAttendance.deleteMany(),
    CommitteeVote.deleteMany(), ManagementVote.deleteMany(),
  ]);
  ...
  const makeUserAndMembership = async ({ name, email, role, phone, flatNo, tower, residentType }) => {
    ...
    await Membership.create({ user: user._id, society: sid, role, flatNo: flatNo || null, tower: tower || null, flatId: flatNo || null });
    return user;
  };
  ...
  // Demonstrate multi-unit ownership: Rahul also owns a second flat in Tower D
  ...
  await Building.insertMany(towers.map((t) => ({ society: sid, name: t })));

  await Unit.updateOne({ society: sid, flatNo: 'A-101' }, { owner: rahul._id, resident: rahul._id, status: 'Occupied' });
  await Unit.updateOne({ society: sid, flatNo: 'A-102' }, { owner: priya._id, resident: priya._id, status: 'Occupied' });
  await Unit.updateOne({ society: sid, flatNo: 'B-201' }, { resident: amit._id, status: 'Occupied' });
  await Unit.updateOne({ society: sid, flatNo: 'B-202' }, { owner: neha._id, resident: neha._id, status: 'Occupied' });
  await Unit.updateOne({ society: sid, flatNo: 'C-301' }, { resident: vikram._id, status: 'Occupied' });
  await Unit.updateOne({ society: sid, flatNo: 'D-401' }, { resident: meera._id, status: 'Occupied' });
  ...
  await Unit.updateOne({ society: sid, flatNo: 'D-402' }, { owner: rahul._id, status: 'Occupied' });
  await Membership.create({ user: rahul._id, society: sid, role: 'resident', flatNo: 'D-402', tower: 'Tower D', flatId: 'D-402' });
  await Membership.create({ user: rahul._id, society: sid, role: 'secretary' });

  console.log('Creating resident profiles...');
  await Resident.insertMany([
    { society: sid, user: rahul._id, flatNo: 'A-101', tower: 'Tower A', type: 'Owner', vehicles: [{ type: 'Car', number: 'MH02 AB 1234' }] },
    { society: sid, user: priya._id, flatNo: 'A-102', tower: 'Tower A', type: 'Owner' },
    { society: sid, user: amit._id, flatNo: 'B-201', tower: 'Tower B', type: 'Tenant' },
    { society: sid, user: neha._id, flatNo: 'B-202', tower: 'Tower B', type: 'Owner' },
    { society: sid, user: vikram._id, flatNo: 'C-301', tower: 'Tower C', type: 'Tenant' },
    { society: sid, user: meera._id, flatNo: 'D-401', tower: 'Tower D', type: 'Tenant' },
  ]);
  ...
  await Complaint.insertMany([
    { society: sid, title: 'Water Leakage in Bathroom', description: 'Water is leaking from the tap in bathroom.', category: 'Plumbing', priority: 'High', status: 'Open', flatNo: 'A-101', raisedBy: rahul._id },
    { society: sid, title: 'Lift Not Working', description: 'Lift of Tower A is not working since morning.', category: 'Lift', priority: 'High', status: 'In Process', flatNo: 'A-102', raisedBy: priya._id },
    { society: sid, title: 'Street Light Not Working', description: 'Street light near gate is not working.', category: 'Electrical', priority: 'Medium', status: 'In Process', flatNo: 'B-201', raisedBy: amit._id },
    { society: sid, title: 'Garbage Not Collected', description: 'Garbage not collected from our floor.', category: 'Housekeeping', priority: 'Low', status: 'Resolved', flatNo: 'B-202', raisedBy: neha._id, resolvedOn: new Date() },
  ]);

  console.log('Creating maintenance requests...');
  await Maintenance.insertMany([
    { society: sid, title: 'Water Leakage in Bathroom', description: 'Water is leaking from the tap.', category: 'Plumbing', priority: 'High', status: 'Open', flatNo: 'A-101', raisedBy: rahul._id },
    { society: sid, title: 'Lift Not Working', description: 'Lift is not working since morning.', category: 'Lift', priority: 'High', status: 'In Progress', flatNo: 'A-102', raisedBy: priya._id },
    { society: sid, title: 'AC Outdoor Unit Noise', description: 'AC is making a loud noise.', category: 'AC/HVAC', priority: 'Low', status: 'Completed', flatNo: 'B-202', raisedBy: neha._id, completedOn: new Date() },
  ]);

  console.log('Creating notices...');
  await Notice.insertMany([
    { society: sid, title: 'Annual Maintenance Charges Due', description: 'This is a reminder that annual maintenance charges for FY 2024-25 are due by 31st May 2024.', category: 'Finance', status: 'Published', createdBy: secretary._id },
    { society: sid, title: 'Water Supply Interruption', description: 'Water supply will be unavailable on Sunday from 10:00 PM to 6:00 AM due to pipeline maintenance.', category: 'Maintenance', building: 'Tower A, Tower B', status: 'Published', createdBy: secretary._id },
    { society: sid, title: 'Community Meeting', description: 'All residents are requested to attend the community meeting on 18th May 2024 at 6:00 PM in the clubhouse.', category: 'Community', status: 'Published', createdBy: secretary._id },
    { society: sid, title: 'New Parking Rules', description: 'Please follow the updated parking guidelines effective from 20th May 2024.', category: 'Rules & Regulations', status: 'Scheduled', scheduledDate: new Date(Date.now() + 7 * 86400000), createdBy: secretary._id },
  ]);
  ...
  await Meeting.insertMany([
    { society: sid, title: 'Annual General Meeting', type: 'General', date: new Date('2024-05-25T10:00:00'), location: 'Conference Room', agenda: 'Financial approval and by-law amendment discussion', createdBy: secretary._id },
    { society: sid, title: 'Executive Committee Meeting', type: 'Committee', date: new Date('2024-05-20T17:00:00'), location: 'Committee Room', createdBy: secretary._id },
    { society: sid, title: 'Building Maintenance Meeting', type: 'Internal', date: new Date('2024-05-30T18:00:00'), location: 'Tower A Meeting Room', createdBy: secretary._id },
  ]);

  console.log('Creating polls...');
  await Poll.insertMany([
    { society: sid, title: 'By-Law Amendment', description: 'Proposed changes to society by-laws', endDate: new Date(Date.now() + 14 * 86400000), votesYes: 45, votesNo: 8, totalEligible: 60, createdBy: chairman._id },
    { society: sid, title: 'Parking Rules Amendment', description: 'New guidelines for visitor parking', endDate: new Date(Date.now() + 3 * 86400000), votesYes: 30, votesNo: 12, totalEligible: 60, createdBy: chairman._id },
  ]);

  console.log('Creating emergencies, camera requests, policies, investments, funds...');
  await Emergency.create({ society: sid, type: 'Security', flatNo: 'A-101', raisedBy: rahul._id, notes: 'Unauthorized vehicle spotted', status: 'Resolved' });
  await CameraRequest.create({ society: sid, area: 'Main Gate', date: new Date('2024-05-17'), time: '08:30', reason: 'Bike scratch incident in parking.', flatNo: 'A-101', requestedBy: rahul._id, status: 'Pending' });
  ...
  const flatOwnerDefs = [
    { flatId: 'A-101', building: 'A', flatNo: '101', ownerNo: 1, firstName: 'Rahul', lastName: 'Sharma', gender: 'Male', mobileNumber: '9876543210', user: rahul._id },
    { flatId: 'A-102', building: 'A', flatNo: '102', ownerNo: 1, firstName: 'Priya', lastName: 'Patel', gender: 'Female', mobileNumber: '9876543211', user: priya._id },
    { flatId: 'B-202', building: 'B', flatNo: '202', ownerNo: 1, firstName: 'Neha', lastName: 'Singh', gender: 'Female', mobileNumber: '9876543213', user: neha._id },
  ];
  ...
  await RoleChecklist.insertMany([
  ...
  const agmMeeting = await Meeting.findOne({ society: sid, title: 'Annual General Meeting' });
  if (agmMeeting) {
    await MeetingAttendance.insertMany([
      { society: sid, meeting: agmMeeting._id, role: 'secretary', flatId: null, user: secretary._id },
      { society: sid, meeting: agmMeeting._id, role: 'chairman', flatId: null, user: chairman._id },
      { society: sid, meeting: agmMeeting._id, role: 'resident', flatId: 'A-101', user: rahul._id },
      { society: sid, meeting: agmMeeting._id, role: 'committee_member', flatId: null, user: committee._id },
    ]);

    await AgendaItem.create({
      society: sid,
      meeting: agmMeeting._id,
      ...
      noOfVotes: 2,
      voters: [chairman._id, secretary._id],
  ...
  await CommitteeVote.insertMany([
  ...
  await ManagementVote.insertMany([
