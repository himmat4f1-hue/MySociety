...
// Auto-generates Unit (flat) rows for a newly-created society based on
// how many buildings/towers and how many flats per building the admin entered
// during signup. Also creates matching Building rows so the Society
...
const provisionUnits = async (societyId, buildingsCount, flatsPerBuilding) => {
  ...
  if (buildingDocs.length) await Building.bulkCreate(buildingDocs);
  if (docs.length) await Unit.bulkCreate(docs);
  return docs.length;
};
