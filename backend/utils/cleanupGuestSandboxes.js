...
const cleanupExpiredGuestSandboxes = async () => {
  const expired = await Society.findAll({ where: { isGuestSandbox: true, expiresAt: { [Op.lt]: new Date() } } });
  ...
  for (const society of expired) {
    for (const modelName of MODEL_NAMES) {
      ...
      await Model.destroy({ where: { society: society.id } });
    }

    const memberships = await Membership.findAll({ where: { society: society.id } });
    const userIds = memberships.map((m) => m.user);
    await Membership.destroy({ where: { society: society.id } });
    // Only delete the guest User accounts themselves (they're single-use, sandbox-only accounts)
    await User.destroy({ where: { id: { [Op.in]: userIds }, email: { [Op.iLike]: '%@sandbox.mysociety.local' } } });

    await Society.destroy({ where: { id: society.id } });
